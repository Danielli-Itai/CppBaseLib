...
//	if(my_handle == NULL)
//	{
//		my_handle = CreateThread( NULL, 0, (LPTHREAD_START_ROUTINE) ThreadRoutine, (void*)Parent, CREATE_SUSPENDED , &my_id);
...
//FW_RESULT	CMicThread::PostMessage(FW_UINT Msg,WPARAM wParam, LPARAM lParam)
//{
//	RES_DECLR(result);
//
//	if(my_id != 0x00)
...
//}
////	Send a user message to the thread.
//FW_RESULT	CMicThread::PostMessage(FW_UINT Msg)
//{
//	RES_DECLR(result);
//
//	RES_CHECK(result, PostMessage(Msg, Msg, 0x00));		//	Palce message in the thread que and return without waiting for processing.
//
//	RES_RETURN(result);		
//}
//
...
//	DWORD ExitCode = 0x00;										// address to receive the thread termination status
//
//	RES_CHECK(result, FALSE != PostMessage(THREAD_STATE_TERMINATE));	//	Send termination message.
