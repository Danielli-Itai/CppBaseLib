...
//----------------------------------------------------------
//	Convert a number written in a string in to an FW_INTeger.
...
STRING	GetTextRecord(STRING page, unsigned char tag, FW_INT index)
{
	...
	for(counter = 0; counter < Length; counter++)
	{
		...
		{														//	result still true.
			...
			{													//	Ignore negative signe
				...
				{												//	Check Hex header
					if((counter > 1) && (num_str.Mid (counter - 1, 1) == _T("0"))){
...
STRING	CMicString::TextRangeRmv(STRING string, unsigned char tag_start, unsigned char tag_end, FW_INT index)
{
	...
	STR_ADD(tmp, tag_start);		//"("
	STR_ADD(tmp, enclosed);		//"(..."
	STR_ADD(tmp, tag_end);		//"(...)"
	STR_SET(enclosed, tmp);

	STR_SET(tmp,string.Left(string.Find(tmp)));
	if((STR_LEN(tmp) + STR_LEN(enclosed)) < STR_LEN(string)){
		STR_ADD(tmp, string.Right(STR_LEN(tmp) + STR_LEN(enclosed)));
	}
	STR_RETURN(tmp);
}
	...
	while((border_text>=0x00) && (border_text<border_new))				//	New text end found befor formation end.
	{
		...
		if((border_text>=0x00) && (border_text<border_new))				//	New text start found befor formation end. 
		{
			...
			if(border_text>border_new){											//	search new formation end after end of text.
				border_new = description.Find(tag, border_text);	// search End of formation end.
				if(border_new < 0x00){
					border_new = STR_LEN(description);
				}
			}
...
STRING	CMicString::RangeRemove(STRING& pDescription, STR_TAG tag, FW_INT index)
{
	...
	if(range.length > 0x00){
		...
		pDescription.Delete(range.from, range.length);				//	Remove the range.
	}
	STR_RETURN(replay);
}
...
//	my_item =	chain;
	...
	{
		border_next--;																//	Remove the end mark and return contents only.
		STR_SET(replay, chain.Mid(border_curr,(border_next-border_curr)));	//	Set as active item.
	}
...
FW_RESULT	CMicString::AddItem(STRING& page, STRING item)
{
	...
	//if(STR_NOT_EMPTY(my_page)){
	//	STR_ADD(my_page, TAG_ITM_END);
	//}
	//STR_ADD(my_page, item);
...
STRING	CMicString::GetItem(STRING page, FW_INT index)
{
	...
	{
		border_next--;																	//	Remove the end mark and return contents only.
//		my_item = my_page.Mid(border_curr,(border_next-border_curr));	//	Set as active item.
//		STR_SET(replay,my_item);															//	return the item.
		STR_SET(replay, page.Mid(border_curr,(border_next-border_curr)));	//	return the item.
	}
...
FW_RESULT	CMicString::AddPage(STRING& segment, STRING page)
{
	...
	//if(STR_NOT_EMPTY(my_segment)){
	//	STR_ADD(my_segment, TAG_PAG_END);
	//}
	//STR_ADD(my_segment, page);
...
STRING	CMicString::GetPage(STRING segment, FW_INT index)
{
	...
	{
		border_next--;																	//	Remove the end mark and return contents only.
		//STR_SET(my_page,my_segment.Mid(border_curr,(border_next-border_curr)));	//	Set as active array.
		//STR_SET(replay,my_page);																//	return the array.
		STR_SET(replay, segment.Mid(border_curr,(border_next-border_curr)));	//	return the array.
	}
	STR_RETURN(replay);
}
...
STRING	CMicString::GetSegment(STRING formation, FW_INT index)
{
	...
	{
		border_next--;																	//	Remove the end mark and return contents only.
		//STR_SET(my_segment, my_formation.Mid(border_curr,(border_next-border_curr)));
		//GetPage(my_segment,0x00);
		//STR_SET(replay,my_segment);
		STR_SET(replay, formation.Mid(border_curr,(border_next-border_curr)));
	}
...
STRING	CMicString::FormationAdd(STRING& description, STRING	formation)
{
	...
	STR_ADD(formation,'\n');
	STR_ADD(formation,'\r');
	//STR_ADD(my_description,formation);
...
STRING	CMicString::FormationGet(STRING description, FW_INT index)
{
	...
	{
		border_next--;																	//	Remove the end mark and return contents only.
		//STR_SET(my_formation,my_description.Mid(border_curr,(border_next-border_curr)));
		//GetSegment(my_formation,0x00);
		//STR_SET(replay,my_formation);

		STR_SET(replay, description.Mid(border_curr,(border_next-border_curr)));
	}
