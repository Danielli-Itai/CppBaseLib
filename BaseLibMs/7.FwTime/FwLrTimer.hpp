...
class LRTimer {
	...
	public:
		...
		VOID setCallbackFunction( VOID (*_pCallback)(PVOID,DWORD,DWORD), PVOID pv);
  ...
  private:
  	...
  	VOID (*m_pCallback)(PVOID,DWORD,DWORD);	// pointer to user callback function
	PVOID m_pv;						// Argument to user callback function
  	BOOL m_bRunning;				// timer running state
	BOOL m_bExitThred;				// Exit thread State
	...
	// timer callback APC procedure called when timer is signaled
 	virtual VOID CALLBACK TimerAPCProc(void*, DWORD, DWORD);
